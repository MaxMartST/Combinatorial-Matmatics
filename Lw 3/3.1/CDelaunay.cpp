...
namespace dt {
	...
	template<typename T>
	const std::vector<typename CDelaunay<T>::TriangleType>& CDelaunay<T>::GetTriangles() const
	{
		return _triangles;
	}

	template<typename T>
	const std::vector<typename CDelaunay<T>::EdgeType>& CDelaunay<T>::GetEdges() const
	{
		return _edges;
	}

	template<typename T>
	const std::vector<typename CDelaunay<T>::VertexType>& CDelaunay<T>::GetVertices() const
	{
		return _vertices;
	}

	template class CDelaunay<float>;
	template class CDelaunay<double>;
