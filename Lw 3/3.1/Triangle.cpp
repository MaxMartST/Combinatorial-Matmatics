...
namespace dt {
	...
	template<typename T>
	bool Triangle<T>::containsVertex(const VertexType &v) const
	{
	...
	template<typename T>
	bool Triangle<T>::operator ==(const Triangle &t) const
	{
	...
	template<typename U>
	std::ostream& operator <<(std::ostream &str, const Triangle<U> &t)
	{
		return str << "Triangle:" << "\n\t" << *t.a->number << "\n\t" << *t.b->number << "\n\t" << *t.c->number << '\n';
	}
